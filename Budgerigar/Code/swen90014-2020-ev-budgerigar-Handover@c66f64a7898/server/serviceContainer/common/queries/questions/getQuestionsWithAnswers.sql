...
LEFT JOIN 
    (SELECT answers.question_id, json_agg(answers) AS answers FROM
        (SELECT a.*, u.user FROM answers AS a
        LEFT JOIN 
           (SELECT users.id, row_to_json(users) AS user FROM users) AS u 
        ON a.user_id = u.id) as answers
    GROUP BY answers.question_id) AS answer_list
ON q.id = answer_list.question_id;